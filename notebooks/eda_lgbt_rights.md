...
# EDA: LGBT Rights and Anti-Discrimination Attitudes

Allen Downey

[MIT License](https://en.wikipedia.org/wiki/MIT_License)

Task 8 exploratory analysis: year coverage, raw code distributions, missing codes, and time trends for selected CDF variables. Question text comes from `codebook/extracted/anes_cdf_minimal.json`.
...
# Task 8 selected variables (see project_board.md)
MAIN_VARS = ["VCF0876", "VCF0877", "VCF0878"]
STRENGTH_VARS = ["VCF0876a", "VCF0877a"]
ATTITUDE_VARS = MAIN_VARS + STRENGTH_VARS
THERMOMETER_VARS = ["VCF0232"]  # already in extract; cross-reference
...
log_path = f"logs/eda_lgbt_rights_{input_stem}.txt"
debug_log = open(log_path, "w")

log_and_print("ANES LGBT rights / anti-discrimination EDA")
...
```python
fig, axes = plt.subplots(len(ATTITUDE_VARS), 1, figsize=(10, 1.8 * len(ATTITUDE_VARS)), sharex=True)
...
fig.suptitle("Valid responses by survey year (Task 8 attitude items)", y=1.01)
plt.tight_layout()
plt.show()
```
...
Main items (`VCF0876`, `VCF0877`, `VCF0878`): 1 = favor / allow / yes, 5 = oppose / don't allow / no. Strength items (`VCF0876a`, `VCF0877a`): 1–2 = favor/allow side, 4–5 = oppose/don't-allow side. Lower mean code = more supportive of LGBT rights on all items.
...
fig, axes = plt.subplots(len(MAIN_VARS), 1, figsize=(10, 3 * len(MAIN_VARS)), sharex=True)
if len(MAIN_VARS) == 1:
    axes = [axes]

for ax, var in zip(axes, MAIN_VARS):
...
axes[-1].set_xlabel("year")
fig.suptitle("Mean main-item code by year (1=supportive, 5=opposed; DK/NA recoded)", y=1.01)
plt.tight_layout()
plt.show()
```

```python
# Share favoring / allowing (code 1) on main items
fig, axes = plt.subplots(1, len(MAIN_VARS), figsize=(5 * len(MAIN_VARS), 4), sharey=True)
if len(MAIN_VARS) == 1:
    axes = [axes]
...
for ax, var in zip(axes, MAIN_VARS):
    ...
    if tmp.empty:
        ax.set_title(f"{var}\n(no data)")
        continue
    share_favor = tmp.groupby("year")["_val"].apply(lambda s: (s == 1).mean() * 100)
    ax.plot(share_favor.index, share_favor.values, marker="o", ms=4)
    ax.set_xlabel("year")
    ax.set_ylabel("% code 1")
    ax.set_title(catalog_entry(var, catalog).get("label", var), fontsize=9)
    ax.set_ylim(0, 100)

fig.suptitle("Share at most-supportive code (1) by year", y=1.02)
plt.tight_layout()
plt.show()
```

## Feeling Thermometer (cross-reference)

`VCF0232` (Gays and Lesbians) is already in the extract; included here for comparison with attitude trends.
...
coverage_path = out_dir / "eda_lgbt_rights_coverage.csv"
summary_path = out_dir / "eda_lgbt_rights_missing_summary.csv"
